...
### selects just years from firstYear to base_year to identify constant set of routes
### for LFO predictions
...
# set up all other data that will be included in future predictions 
route_df = unique(data.frame(route = new_data$route,
                              routeF = new_data$routeF))#selected routes for predictions and CV

#prepare data for rest of time series
full_data_jags <- prepare_data(strat_data = strat_data,
                                      species_to_run = species,
                                      model = model,
                                      #n_knots = 10,
                                      min_year = firstYear,
...
#select just the future data that matches the routes with data up to baseYear
full_data <- data.frame(strat_name = full_data_jags$strat_name,
                       strat = full_data_jags$strat,
                       route = full_data_jags$route,
                       strat = full_data_jags$strat_name,
                       Latitude = full_data_jags$Latitude,
                       Longitude = full_data_jags$Longitude,
                       count = full_data_jags$count,
                       year = full_data_jags$year,
                       firstyr = full_data_jags$firstyr,
                       ObsN = full_data_jags$ObsN,
                       r_year = full_data_jags$r_year) %>% 
  filter(route %in% new_data$route) %>% 
  inner_join(.,route_df,by = "route")
